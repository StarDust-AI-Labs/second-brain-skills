...
`route-contracts.json` 是场景链路的**唯一规范源**。Hub 在完成意图识别后，必须读取该文件并写入 Ledger：

1. 将 `global_preflight` 写入 `hub_run_ledger.global_preflight`，并先完成 Vault 配置与意图确认。
2. 按场景 `id` 选择一条 `scenes` 契约，写入 `hub_run_ledger.scenario_contract`。
3. 将该契约的 `required_steps` 原样写入 `hub_run_ledger.required_chain`；不得从场景正文或记忆中自行补写、删减或改序。
4. 对 `conditional_steps`，满足 `run_when` 时执行并记录输出；不满足时，将 `id` 和 `skip_evidence` 写入 `optional_steps_skipped`。
5. 对 `mode` 为 `write`、`update`、`move-or-delete` 的场景，完成 `write_preflight` 后才能执行副作用操作。
...
second-brain-hub (本 skill)
  ...
  ├── 场景4(创作)  → [intermediate-packets] → [obsidian-cli search] → [progressive-summarization(L2, 条件)] → [creative-workflow] → [obsidian-markdown] → [obsidian-cli create]
