...
2. 完整读取 `route-contracts.json` 和 `capability-contracts.json`。
...
未读取两份契约并把场景的 `required_steps` 写入运行台账前，不得开始场景执行。
</HARD-GATE>
...
| 能力 | 文件 |
|---|---|
| 保存价值判断 | [references/module-capture-criteria.md](references/module-capture-criteria.md) |
| PARA 归属 | [references/module-para-system.md](references/module-para-system.md) |
| 渐进式提炼 | [references/module-progressive-summarization.md](references/module-progressive-summarization.md) |
| 半熟素材识别 | [references/module-intermediate-packets.md](references/module-intermediate-packets.md) |
| 创作工作流 | [references/module-creative-workflow.md](references/module-creative-workflow.md) |
| 知识生命周期 | [references/module-knowledge-lifecycle.md](references/module-knowledge-lifecycle.md) |
| 发散/聚合诊断 | [references/module-diverge-converge.md](references/module-diverge-converge.md) |
| 长期兴趣匹配 | [references/module-twelve-favorite-problems.md](references/module-twelve-favorite-problems.md) |
| CODE 系统诊断 | [references/module-code-diagnosis.md](references/module-code-diagnosis.md) |
| 综合诊断器 | [references/module-second-brain-diagnosis.md](references/module-second-brain-diagnosis.md) |

用户要求方法论原文、案例或历史审计时，读取 [references/methodology-sources.md](references/methodology-sources.md)。这些档案不是运行时入口。

`defuddle`、`obsidian-markdown` 和 `obsidian-cli` 保持外部工具 Skill；需要时读取其 `SKILL.md`，不要复制其技术说明到本 Skill。
...
- 工具不可用：按能力契约的 `failure_mode` 降级或停止，不得静默跳过方法论步骤。
