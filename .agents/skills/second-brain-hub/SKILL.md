...
1. ✅ 已调用 para-system 确认目标文件夹路径（不得写入 Vault 根目录）
2. ✅ 笔记内容已套用 4.1 标准模板（frontmatter 六字段齐全）
3. ✅ status 字段设为 "inbox"（非 "processed" 或 "distilled"，除非经过完整提炼流程）
4. ✅ 文件名格式符合 "标题_YYYY-MM-DD.md"（灵感类加 HHmm）
...
second-brain-hub (本 skill)
  ...
  ├── 场景5(回顾)  → [knowledge-lifecycle] → [obsidian-cli list/search] → [obsidian-cli create]
