...
- `.claude/hub-state.json`（v1.1 时的本地运行态状态；后续安装包只提交 `.claude/hub-state.example.json`）
...
- 真实 `vault_path` 不应进入可安装 Skill 包，应使用 `.claude/hub-state.example.json` + 本地 `.claude/hub-state.json`。
